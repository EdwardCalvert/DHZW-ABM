...
public class RoutingTrainReader {
    ...
    private final HashMap<String, HashMap<String, Double>> waitingTimes;
    ...
    public Double getWaitingTime(String location1, String location2) {
        return this.waitingTimes.get(location1).get(location2);
    }
    ...
    private void readTravelTimes(File routingFile) {
        ...
        try {
            ...
            while ((line = reader.readNext()) != null) {
                ...
                double waitingTime = Double.parseDouble(line[12].trim());
                ...
                if(this.trainTimes.containsKey(departure)) {
                    ...
                    this.waitingTimes.get(departure).put(arrival,waitingTime);
                    ...
                    HashMap <String, Double> newTrainTimes = new HashMap<>();
                    newTrainTimes.put(arrival, trainTime);
                    this.trainTimes.put(departure, newTrainTimes);

                    HashMap <String, Double> newWaitingTimes = new HashMap<>();
                    newWaitingTimes.put(arrival, waitingTime);
                    this.waitingTimes.put(departure,newWaitingTimes);

                    HashMap <String, Double> newTrainDistances = new HashMap<>();
                    newTrainDistances.put(arrival, trainDistance);
                    this.trainDistances.put(departure, newTrainDistances);

                    HashMap <String, Double> newWalkTimes = new HashMap<>();
                    newWalkTimes.put(arrival, walkTime);
                    this.walkTimes.put(departure, newWalkTimes);

                    HashMap <String, Double> newBusTimes = new HashMap<>();
                    newBusTimes.put(arrival, busTime);
                    this.busTimes.put(departure, newBusTimes);

                    HashMap <String, Double> newBusDistances = new HashMap<>();
                    newBusDistances.put(arrival, busDistance);
                    this.busDistances.put(departure, newBusDistances);
                    ...
                    HashMap <String, Integer> newFeasibleFlags = new HashMap<String, Integer>();
                    newFeasibleFlags.put(arrival, feasibleFlag);
                    this.feasibleFlags.put(departure, newFeasibleFlags);

                    HashMap <String, Double> newTotalDistances = new HashMap<>();
                    newTotalDistances.put(arrival, distanceTotal);
                    this.totalDistances.put(departure, newTotalDistances);
                }
