run_generate_OD <- function(
  output_dir,
  final_activities_locations_csv,
  centroids_pc5_DHZW_csv
) {
  df_activities <- read.csv(final_activities_locations_csv)
  ...
  df_PC5 <- read.csv(centroids_pc5_DHZW_csv)
  ...
  OD_symmetric_csv <- file.path(output_dir, "OD_symmetric.csv")
  write.csv(df_symmetric, OD_symmetric_csv, row.names = FALSE)

  OD_asymmetric_csv <- file.path(output_dir, "OD_asymmetric.csv")
  write.csv(df_asymmetric, OD_asymmetric_csv, row.names = FALSE)

  return(c(OD_symmetric_csv, OD_asymmetric_csv))
}
