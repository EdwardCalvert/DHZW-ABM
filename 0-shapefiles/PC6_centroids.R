...
generate_pc6_centroids <- function(output_dir, pc4_codes_csv, pc6_gpkg){
  ...
  centroids_pc6_shp = file.path(output_dir, 'centroids_PC6_NL_shp.shp')
  write_sf(df_PC6_NL, centroids_pc6_shp, overwrite=TRUE)

  centroids_pc6_csv = file.path(output_dir,'centroids_PC6_NL.csv' )
  write.csv(df_PC6_NL_coordinates, centroids_pc6_csv, row.names = FALSE)
