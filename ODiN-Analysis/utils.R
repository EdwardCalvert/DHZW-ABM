...
filter_attributes_OViN <- function(df) {
  ...
  if (hasName(df, "KHvm")) {
    df$TransportRole <- df$KHvm
  } else if (hasName(df, "KRvm")) {
    df$TransportRole <- df$KRvm
  } else {
    stop(paste0("Could not infer role in transport", names(df)))
  }
