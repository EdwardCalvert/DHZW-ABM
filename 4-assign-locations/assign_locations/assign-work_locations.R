assign_work_locations <- function(
  df_activities,
  ODiN_work_act_prop_csv,
  ...
  print("processing work locations")
  # Load PC4 proportion destinations
  df_work_prop <- read.table(ODiN_work_act_prop_csv, check.names = FALSE, sep = ",", header = TRUE)
  ...
  # load work locations
  df_work_locations <- read.csv(work_locations_csv)
  ...
  print("work activity assignment complete.")
