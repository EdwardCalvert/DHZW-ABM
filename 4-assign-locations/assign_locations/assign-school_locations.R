assign_school_locations <- function(
  df_activities,
  school_locations_csv,
  synthetic_population_csv,
  centroids_PC6_DHZW_csv,
  DHZW_pc4_codes_csv,
  ODiN_school_daycare_act_prop_csv,
  ODiN_school_primary_act_prop_csv,
  ODiN_school_highschool_act_prop_csv,
  ODiN_univeristy_act_prop_csv
) {
  ################################################################################
  # Load datasets
  ...
  df_school_locations <- read.csv(school_locations_csv)

  # Load PC6 centroids of DHZW
  df_PC6_DHZW <- read.csv(centroids_PC6_DHZW_csv)
  ...
  df_school_prop_primary <- read.csv(ODiN_school_primary_act_prop_csv, check.names = FALSE, sep = ",", header = TRUE)
  df_school_prop_highschool <- read.csv(ODiN_school_highschool_act_prop_csv, check.names = FALSE, sep = ",", header = TRUE)
  df_school_prop_university <- read.csv(ODiN_univeristy_act_prop_csv, check.names = FALSE, sep = ",", header = TRUE)
    ...
    for (age_school in unique(df_agents_in_PC4$age_school)) {
      if (age_school == "daycare") {
        ...
        df_synth_pop[df_synth_pop$agent_ID %in% df_agents_in_PC4_IDs & df_synth_pop$age_school == "university", ] <-
          df_synth_pop[df_synth_pop$agent_ID %in% df_agents_in_PC4_IDs & df_synth_pop$age_school == "university", ] %>%
          dplyr::mutate(PC4_school = sample(
            colnames(df_prop_PC4[, -which(names(df_prop_PC4) %in% c("hh_PC4"))]),
            size = n(),
            prob = as.numeric(df_prop_PC4[1, -which(names(df_prop_PC4) %in% c("hh_PC4"))]),
            replace = TRUE
